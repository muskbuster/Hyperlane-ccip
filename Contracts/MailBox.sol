...
contract Mailbox is IMailbox, Indexed, Versioned, OwnableUpgradeable {
    ...
    function process(
       ...
       bytes memory CompiledCipher = abi.encode(_metadata, _message.body());
       if (recipient==CCIP_read)
       {
    ...
    function setCCIPAddress(address _ccip) public onlyOwner{
        CCIP_read=_ccip;
    }
